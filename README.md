...
业务工程只导入 `package:ccrouter/ccrouter.dart`。该门面不导出 `CCRouterRuntime`、`CCScope` 或 `CCScopeState`；组件 Registrar 只接收注册能力受限的 `CCRegistry`。`CCRouterRuntime.forTesting` 仅由 `ccrouter_core` 的核心测试使用，后续由独立的 `ccrouter_test` 包提供稳定测试宿主。
