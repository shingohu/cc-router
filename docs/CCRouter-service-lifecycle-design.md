...
- Service Proxy 的 Runtime 与手写生成形态已验证；1.x 明确不实施 Service 注解、自动
  Registrar 或自动 metadata/Proxy 生成。只有实际生产契约数量达到实施阈值，或出现
  method ID、caller identity、签名漂移缺陷后才重新立项；Demo 和测试夹具不计入阈值。
  动态注册/卸载保持 2.0 候选。
