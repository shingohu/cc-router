...
以下约定是后续设计、实现、评审和生成器开发的共同标准。新增能力如果无法说明
它如何满足这些约定，不能仅因为“可以实现”就进入公开 API。

1. **智能（自动配置优先，显式覆盖兜底）**
   - 默认值、组件装配、路由表聚合、参数 Codec、基础诊断和适配器能力尽量由生成器或 Runtime 自动完成。
   - 自动行为必须可预测、可导出、可诊断；不能依赖隐式全局状态、运行时反射或不可解释的启发式。
   - 业务和宿主可以在明确边界内覆盖默认配置，覆盖结果必须经过同样的校验。

2. **简单易用（低成本上手，复杂能力渐进暴露）**
   - 常见场景使用少量稳定 API 即可完成：初始化、组件注册和 `CCRouter.navigator` 导航。
   - 高级能力通过可选包、生成代码和 Adapter SPI 增量接入，不把 Shell、拦截器、Trace 或多窗口配置强加给简单应用。
   - API 命名、错误和生命周期保持一致，避免为相近语义创建多套调用方式。

3. **功能强大（覆盖真实场景，但不制造万能接口）**
   - 能力优先围绕组件协作、类型安全路由、生命周期、拦截、Deep Link、诊断和可替换后端建设。
   - 每个 API 只表达一个清晰语义；不使用同时承担导航、状态、权限和业务通信的万能 `Map` 或字符串接口。

4. **可扩展性强（稳定契约，替换实现）**
   - Core 依赖中立契约，导航后端、生成器、诊断导出和平台集成通过明确 SPI 扩展。
   - 扩展点必须定义能力探测、生命周期、错误和线程/异步语义，不能通过访问内部字段实现“扩展”。
   - 新后端消费同一份 Route Definition，不复制一套不同的业务路由模型。

5. **可测试（确定性优先，测试不依赖真实 UI）**
   - Runtime、Registry、Codec、拦截器和生命周期可以在 Pure Dart 测试中运行。
   - Adapter、Clock、Trace/Telemetry 和外部输入可注入或替换；测试必须能验证顺序、取消、超时、重定向和并发边界。
   - 面向宿主和组件作者的测试工具统一放入 `ccrouter_test`，不要求业务导入 Core 私有实现。

6. **尽量少开放不必要的 API（最小权限和最小表面积）**
   - 业务只依赖 `CCRouter`、生成的类型安全契约和明确的只读诊断类型。
   - Runtime、Registry 内部表、可变 RouteEntry、Adapter 控制器、Scope 和销毁入口默认隐藏。
...
8. **非侵入式（接入现有工程，不接管应用）**
   - 不强制页面继承基类、混入特定 State、保存全局 `BuildContext` 或替换应用的状态管理、网络和 UI 体系。
   - 简单工程可以继续使用自己的 `MaterialApp.router` 和 `GoRouter`；需要 Host 能力时再选用 `CCRouterApp`。
   - Adapter 不销毁应用拥有的后端对象，组件也不能绕过 `CCRouter.navigator` 直接改变框架语义。
...
11. **可观测、可诊断、可溯源（默认安全且有界）**
    ...
    - Trace、Telemetry、日志和错误使用不同数据边界，默认只记录白名单字段、类型和安全摘要，不记录 Extra、Token、完整 URI 或业务返回对象。
    - 诊断缓存有界，Observer/Telemetry listener 的异常不能改变业务结果；失败、取消、超时、重定向和降级必须可区分。

12. **并发安全（确定性、可取消、不可重入）**
    - Runtime 的注册、初始化、销毁、Session 切换和 Adapter 生命周期必须串行化并具备幂等或明确拒绝语义。
    - 导航和拦截器支持异步执行、取消和超时；同一请求的重定向链共享稳定 ID，禁止同步重入 Adapter 或在回调中递归导航。
    - 共享状态只通过受控队列、不可变快照或单一 Owner 修改；并发调用的顺序、冲突和结果必须可测试、可诊断。

13. **注重性能和稳定（可量化、可退化、少抖动）**
    - 启动、组件装配、路由解析和常规导航的热路径应避免反射、重复扫描、无界分配和不必要的异步切换；生成结果和稳定索引优先在构建期准备。
    - Trace、Telemetry、Observer、缓存和事件队列必须有界，监听器或诊断导出不能阻塞或改变业务主流程；高负载时允许采样、丢弃非关键诊断，但不能丢失核心错误和生命周期状态。
    - 所有性能优化必须以行为等价、错误可见和可回退为前提，不能通过缓存脏数据、吞掉异常或改变生命周期语义换取表面速度。
    - 关键路径建立基准和回归指标，至少覆盖冷启动、首次初始化、路由解析、导航延迟、内存增长、并发导航和销毁耗时；版本升级或 Adapter 替换后必须重新验证。

### 3.2 开发评审清单

每项新能力在合入前至少回答以下问题：

- 默认路径是否足够简单，自动行为是否可解释并可显式覆盖？
- 是否新增了必要的公开 API，还是可以通过生成代码、内部扩展或已有契约完成？
- 是否有编译期校验、类型安全边界和不支持场景的明确错误？
- 是否明确了 Owner、Scope、初始化/销毁顺序、失败和取消语义？
- 是否支持 Pure Dart/替身测试，并覆盖顺序、重入、并发和降级边界？
- 是否产生可关联、脱敏、有界的诊断信息？
- 是否评估了启动、热路径、内存、并发和销毁性能，并具备稳定性回归指标与安全退化策略？
...
诊断快照；Runtime dispose 时解除订阅。外部 Deep Link 仍由 Core 先执行 Ingress 与
Deep Link Policy，之后才通过 Adapter 的 `go` 进入目标 Shell 分支。
